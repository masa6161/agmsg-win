...
@test "watch-once: exits 2 on timeout when no unread inbound exists" {
  run bash "$SCRIPTS/codex/watch-once.sh" "$PROJ" codex --name alice --team team --timeout 1 --interval 1
  [ "$status" -eq 2 ]
...
@test "watch-once: detects a message that arrives after it starts" {
  bash "$SCRIPTS/codex/watch-once.sh" "$PROJ" codex --name alice --team team --timeout 5 --interval 1 \
